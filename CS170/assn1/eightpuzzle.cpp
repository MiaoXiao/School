...
bool generalSearch(State initial, int h)
{
	...
	while (!avaliableStates.empty())
	{
		...
		//For each operation, also check to see if this operation is a reverse of the last operation, if so do not check this state.
		//Display info about currently expanded node
		...
		//check if this is the max size of queue
	...
	//if no more items on queue, puzzle impossible
	cout << "Total Nodes Expanded: " << totalNodesExpanded << endl;
	cout << "Max Nodes in Queue: " << maxNodesInQueue << endl;
	cout << "Depth of Goal: " << goalDepth << endl;
